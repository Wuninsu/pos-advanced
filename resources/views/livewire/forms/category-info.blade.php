                    ...
                    <div class="dropdown-menu p-0 mt-1" id="searchResults">
                        <div class="list-group">
                            @forelse ($categories as $item)
                                <a href="#" wire:click="findCategory('{{ $item->id }}')"
        ...
        <div class="py-6">
            <div class="card p-3 mb-3">
                <span>Products List For <strong
                        class="text-danger">{{ $cproducts[0]->category->name ?? 'N/A' }}</strong> Category; Total Count: <span
                        class="badge bg-primary"> {{ count($cproducts ?? []) }}</span> </span>
            </div>
            <div class="row">
                @forelse ($cproducts as $item)
